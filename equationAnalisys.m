...
%%
close all; clc;
figure('Name','SS-calculation, wecSim-calculation and KF-estimation excitaion moment','Units','Normalized','OuterPosition', [0 0 1 1] );
plot( time, exM, 'k' );  hold on;   grid on;    grid minor;  box on;
plot(computedExMoment.time,computedExMoment.signals.values,'r.')
estimated_exMoment = squeeze(estimated_states.signals.values(5,1,:));
plot(estimated_states.time,estimated_exMoment, 'b-')
...
k_ini=1;          %Starting Point of For Loop
   for k=k_ini:kT-1
       Uk(:,k)=-16.7*Xk(2,k);                   %No control      
       %System Simulation
       Xk(:,k+1) = Ad*Xk(:,k) + Bd*Uk(:,k) - Bd*exM(k,1);    %States
